				     ...
				     struct nfs_resop4 *resp)
{
	...
	res_GETXATTR4->status = NFS4_OK;
	res_GETXATTR4->GETXATTR4res_u.resok4.gxr_value.utf8string_len =
						gxr_value.utf8string_len;
	res_GETXATTR4->GETXATTR4res_u.resok4.gxr_value.utf8string_val =
				     ...
				     struct nfs_resop4 *resp)
{
	...
	LogDebug(COMPONENT_NFS_V4,
		 "SetXattr option=%d key=%.*s value=%.*s",
		 arg_SETXATTR4->sxa_option,
		 arg_SETXATTR4->sxa_key.utf8string_len,
		 arg_SETXATTR4->sxa_key.utf8string_val,
		 arg_SETXATTR4->sxa_value.utf8string_len,
		 arg_SETXATTR4->sxa_value.utf8string_val);
...
void nfs4_op_listxattr_Free(nfs_resop4 *resp)
{
	...
	if (res_LISTXATTR4->status == NFS4_OK) {
		gsh_free(res->lxr_names.xl4_entries);
	}
}
